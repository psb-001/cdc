...
export default function Team() {
  return (
    <section id="team" className="section" style={{ background: '#F8FAFC' }}>
      <div className="container">
        <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
          <div className="section-label">Our Leadership</div>
          <h2 style={{ fontSize: '2.5rem', marginBottom: '1rem', color: '#0F172A' }}>The CDC Team</h2>
        ...
        <div className="grid-4">
          {members.map((m, i) => (
            <motion.div key={m.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <h3 style={{ fontSize: '1.1rem', marginBottom: '0.25rem', color: '#0F172A' }}>{m.name}</h3>
